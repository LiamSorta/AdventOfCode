...
//Function prototype for CheckChar
void CheckChar(char dir, int &x, int &y);

int main(){
	//Saved locations where gifts have been given
	vector<string> savedSanta;
	//Current x/y coordinates for Santa/RoboSanta
	...
	for (int i = 0; i < input.size(); i++){
		...
		//If it's Santa's turn to move, use this, otherwise, it's over to RoboClause
		if (Santa){
			strPos = to_string(xSanta) + "," + to_string(ySanta);
			CheckChar(input[i], xSanta, ySanta);
		}
		else{
			strPos = to_string(xRobo) + "," + to_string(yRobo);
			CheckChar(input[i], xRobo, yRobo);
		}

		//search 'saved' vector locations, if unique, add to the uniqueHouses count
		if (find(savedSanta.begin(), savedSanta.end(), strPos) == savedSanta.end()){
			uniqueHouses++;
		}

		savedSanta.push_back(strPos);
		Santa = !Santa;
...
//Translates character to position
void CheckChar(char dir, int &x, int &y){
	switch (dir)
	{
	case '>':
